// update : 12/16 05:31 -- FUN_BASKET_VIEW 에서 stock이 남은 재고만 표현하도록 수정
// update : 12/15 08:23 -- FUN_BUY 테이블 POINT칼럼 이름을 USEPOINT로 변경
...
SELECT B.SEQ, B.ID, B.PROJECTSEQ, B.OPTIONSEQ, B.COUNT, B.REGDATE,
    ...
    --(SELECT STOCK-BUYCOUNT FROM FUN_OPTION_VIEW WHERE SEQ = B.OPTIONSEQ)
    (SELECT DECODE(SIGN(STOCK-BUYCOUNT),-1,0,STOCK-BUYCOUNT) FROM FUN_OPTION_VIEW WHERE SEQ = B.OPTIONSEQ) -- 0보다 작으면 -1 (무제한인 경우)
FROM FUN_BASKET B;
